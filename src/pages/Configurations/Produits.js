...
import {
    descriptionChanged, itemsChanged, idChanged, produitErrors,
...
const Produits = (props) => {
    ...
    const KPI_CONFIG   = [{ key: "total", label: "Total produits", icon: AssignmentIcon, iconBg: "#DBEAFE", iconColor: "#1D4ED8", borderColor: "#3B82F6", filter: () => true }];
    ...
    const pdfConfig = {
        page_size: 15, filename: "Produits",
    ...
    return (
        <>
            <div className="card-panel">
                <AddDuplicateFormModal open={addModalOpen} onClose={() => setAddModalOpen(false)} title="Ajouter des produits" fields={addFields} onSubmit={handleModalSubmit} loading={addLoading} maxWidth="md" addLabel="Ajouter un autre produit" />
                ...
                <Dialog open={editModalOpen} onClose={() => { if (!editLoading) { setEditModalOpen(false); clearComponentState(); } }} fullWidth maxWidth="sm" PaperProps={{ style: { borderRadius: 16, overflow: "hidden" } }}>
                    <div style={{ background: "linear-gradient(135deg, var(--gpr-primary-dark) 0%, var(--gpr-primary) 100%)", padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            ...
                            <div><div style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>Modifier le produit</div><div style={{ color: "rgba(255,255,255,0.7)", fontSize: 11.5, marginTop: 2 }}>{editForm.libelle}</div></div>
                        </div>
                    ...
                    <DialogContent sx={{ p: 3 }}>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16 }}>
                            <div>
                                <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#475569", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.4px" }}>Nom du produit <span style={{ color: "#ef4444" }}>*</span></label>
                ...
                <Dialog open={deleteConfirm.open} onClose={() => { if (!deleteConfirm.loading) setDeleteConfirm({ open: false, item: null, loading: false }); }} maxWidth="xs" fullWidth PaperProps={{ style: { borderRadius: 16, overflow: "hidden" } }}>
                    <div style={{ background: "linear-gradient(135deg, #991b1b 0%, #ef4444 100%)", padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            ...
                            <div><div style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>Supprimer le produit</div><div style={{ color: "rgba(255,255,255,0.75)", fontSize: 11.5, marginTop: 2 }}>{deleteConfirm.item?.libelle}</div></div>
                        </div>
                ...
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2, flexWrap: "nowrap", gap: 1 }}>
                    <Typography sx={{ fontWeight: 700, fontSize: "0.95rem", color: "#0F172A", flexShrink: 0 }}>Liste des produits</Typography>
                    <Box sx={{ display: "flex", gap: 1, alignItems: "center", flexShrink: 0 }}>
                        ...
                        <Tooltip title="Exporter en Excel"><IconButton onClick={() => table2XLSX("Liste_des_produits" + today().replaceAll("/", ""), "app-produits")} sx={{ border: "1px solid #e2e8f0", borderRadius: 2, color: "#16a34a", "&:hover": { background: "#dcfce7" } }} size="small"><GridOn fontSize="small" /></IconButton></Tooltip>
...
const mapStateToProps = (state) => ({
    isLoading: state.produit.isLoading, id: state.produit.id, libelle: state.produit.libelle,
    description: state.produit.description, items: state.produit.items,
    selectedItem: state.produit.selectedItem, errors: state.produit.produit_errors,
    etat: state.produit.etat, etat2: state.produit.etat2, etat3: state.produit.etat3,
});
const mapDispatchToProps = (dispatch) => ({
    produitErrors: (err) => dispatch(produitErrors(err)),
