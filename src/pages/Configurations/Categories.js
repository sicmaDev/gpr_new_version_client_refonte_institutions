...
import {
    ...
    itemsChanged, categorieErrors, libelleChanged, selectedItemChanged
...
const Categories = (props) => {
    ...
    const KPI_CONFIG   = [{ key: "total", label: "Total catégories", icon: AssignmentIcon, iconBg: "#DBEAFE", iconColor: "#1D4ED8", borderColor: "#3B82F6", filter: () => true }];
        ...
        <>
            <div className="card-panel">
                ...
                <AddDuplicateFormModal
                    ...
                    title="Ajouter des catégories" fields={addFields}
                    onSubmit={handleModalSubmit} loading={addLoading}
                    maxWidth="md" addLabel="Ajouter une autre catégorie"
                />
                ...
                <Dialog open={editModalOpen} onClose={() => { if (!editLoading) { setEditModalOpen(false); clearComponentState(); } }} fullWidth maxWidth="sm" PaperProps={{ style: { borderRadius: 16, overflow: "hidden" } }}>
                    <div style={{ background: "linear-gradient(135deg, var(--gpr-primary-dark) 0%, var(--gpr-primary) 100%)", padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            ...
                            <div>
                                <div style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>Modifier la catégorie</div>
                    ...
                    <DialogContent sx={{ p: 3 }}>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16 }}>
                            ...
                            <div style={{ gridColumn: "1 / -1" }}>
                                ...
                                <textarea value={editForm.description} onChange={(e) => setEditForm(p => ({ ...p, description: e.target.value }))} placeholder="Description de la catégorie" rows={3} style={{ width: "100%", boxSizing: "border-box", resize: "vertical", border: "1.5px solid #e2e8f0", borderRadius: 9, padding: "9px 12px", fontSize: 13, outline: "none", background: "#fff", color: "#1e293b", whiteSpace: "pre-wrap", wordBreak: "break-word", fontFamily: "inherit" }} onFocus={(e) => { e.target.style.borderColor = "var(--gpr-primary)"; }} onBlur={(e) => { e.target.style.borderColor = "#e2e8f0"; }} />
                            </div>
                        </div>
                    </DialogContent>
                ...
                <Dialog open={deleteConfirm.open} onClose={() => { if (!deleteConfirm.loading) setDeleteConfirm({ open: false, item: null, loading: false }); }} maxWidth="xs" fullWidth PaperProps={{ style: { borderRadius: 16, overflow: "hidden" } }}>
                    <div style={{ background: "linear-gradient(135deg, #991b1b 0%, #ef4444 100%)", padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            ...
                            <div>
                                <div style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>Supprimer la catégorie</div>
                ...
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2, flexWrap: "nowrap", gap: 1 }}>
                    <Typography sx={{ fontWeight: 700, fontSize: "0.95rem", color: "#0F172A", flexShrink: 0 }}>Liste des catégories</Typography>
                    <Box sx={{ display: "flex", gap: 1, alignItems: "center", flexShrink: 0 }}>
                        <Tooltip title="Exporter en PDF"><IconButton onClick={() => handlePrint(pdfConfig, pdfColumns, props.items, 0)} sx={{ border: "1px solid #e2e8f0", borderRadius: 2, color: "#ef4444", "&:hover": { background: "#fee2e2", borderColor: "#fca5a5" } }} size="small"><PictureAsPdf fontSize="small" /></IconButton></Tooltip>
                        <Tooltip title="Exporter en Excel"><IconButton onClick={() => table2XLSX("Liste_des_categories" + today().replaceAll("/", ""), "app-categories")} sx={{ border: "1px solid #e2e8f0", borderRadius: 2, color: "#16a34a", "&:hover": { background: "#dcfce7", borderColor: "#86efac" } }} size="small"><GridOn fontSize="small" /></IconButton></Tooltip>
...
const mapDispatchToProps = (dispatch) => ({
    categorieErrors: (err)        => dispatch(categorieErrors(err)),
