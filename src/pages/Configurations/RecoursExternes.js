...
import {
    descriptionChanged, idChanged, itemsChanged, recoursExternesErrors,
...
const RecoursExternes = (props) => {
    ...
    const KPI_CONFIG   = [{ key: "total", label: "Total recours externes", icon: AssignmentIcon, iconBg: "#DBEAFE", iconColor: "#1D4ED8", borderColor: "#3B82F6", filter: () => true }];
    ...
    const pdfConfig = {
        page_size: 15, filename: "Recours Externes",
    ...
    const addFields = [
        { key: "libelle", label: "Intitulé", required: true, fullWidth: false, placeholder: "Ex: Tribunal, Médiateur..." },
        { key: "description", label: "Description", type: "textarea", fullWidth: true, placeholder: "Description du recours" },
    ];
    ...
    return (
        <>
            <div className="card-panel">
                <AddDuplicateFormModal open={addModalOpen} onClose={() => setAddModalOpen(false)} title="Ajouter des recours externes" fields={addFields} onSubmit={handleModalSubmit} loading={addLoading} maxWidth="md" addLabel="Ajouter un autre recours" />
                ...
                <Dialog open={editModalOpen} onClose={() => { if (!editLoading) { setEditModalOpen(false); clearComponentState(); } }} fullWidth maxWidth="sm" PaperProps={{ style: { borderRadius: 16, overflow: "hidden" } }}>
                    <div style={{ background: "linear-gradient(135deg, var(--gpr-primary-dark) 0%, var(--gpr-primary) 100%)", padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            ...
                            <div><div style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>Modifier le recours externe</div><div style={{ color: "rgba(255,255,255,0.7)", fontSize: 11.5, marginTop: 2 }}>{editForm.libelle}</div></div>
                        </div>
                ...
                <Dialog open={deleteConfirm.open} onClose={() => { if (!deleteConfirm.loading) setDeleteConfirm({ open: false, item: null, loading: false }); }} maxWidth="xs" fullWidth PaperProps={{ style: { borderRadius: 16, overflow: "hidden" } }}>
                    <div style={{ background: "linear-gradient(135deg, #991b1b 0%, #ef4444 100%)", padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            ...
                            <div><div style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>Supprimer le recours externe</div><div style={{ color: "rgba(255,255,255,0.75)", fontSize: 11.5, marginTop: 2 }}>{deleteConfirm.item?.libelle}</div></div>
                        </div>
                ...
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2, flexWrap: "nowrap", gap: 1 }}>
                    <Typography sx={{ fontWeight: 700, fontSize: "0.95rem", color: "#0F172A", flexShrink: 0 }}>Liste des recours externes</Typography>
                    <Box sx={{ display: "flex", gap: 1, alignItems: "center", flexShrink: 0 }}>
                        ...
                        <Tooltip title="Exporter en Excel"><IconButton onClick={() => table2XLSX("Liste_des_recours_externes" + today().replaceAll("/", ""), "app-recours")} sx={{ border: "1px solid #e2e8f0", borderRadius: 2, color: "#16a34a", "&:hover": { background: "#dcfce7" } }} size="small"><GridOn fontSize="small" /></IconButton></Tooltip>
                ...
                {viewMode === "list" ? (
                    <ConfigTable items={filteredItems} exportClassName="app-recours" columns={[
...
const mapStateToProps = (state) => ({
    isLoading: state.recoursExternes.isLoading, id: state.recoursExternes.id, libelle: state.recoursExternes.libelle,
    description: state.recoursExternes.description, items: state.recoursExternes.items,
    selectedItem: state.recoursExternes.selectedItem, errors: state.recoursExternes.recoursExternesErrors,
    etat: state.recoursExternes.etat, etat2: state.recoursExternes.etat2, etat3: state.recoursExternes.etat3,
});
const mapDispatchToProps = (dispatch) => ({
    recoursExternesErrors: (err) => dispatch(recoursExternesErrors(err)),
...
export default connect(mapStateToProps, mapDispatchToProps)(RecoursExternes);
