...
const EmailDialog = (props) => {
  ...
  const handleSend = (e) => {
    if (props.maxDelai >= props.handled_delai) {
      ...
      props.handleSubmitConfirmation(e);
    } else {
    ...
    >
      ...
      <DialogContent sx={{ pt: 3, pb: 1 }}>
        <div style={{ marginBottom: 20 }}>
          ...
          <input
            type="number" value={props.handled_delai}
            onChange={(e) => props.handledDelaiChanged(e.target.value)}
            id="te" name="te" max={props.maxDelai} min={1}
          ...
          />
          {errors.maxDelai && <div style={{ fontSize: 12, color: '#ef4444', marginTop: 6 }}>Le délai doit être entre 1 et {props.maxDelai} jour(s)</div>}
        </div>
        {props.handled_custom_message && (
          <div style={{ marginBottom: 16 }}>
            ...
            <TextField multiline fullWidth rows={8} value={props.handled_message} onChange={(e) => props.handledMessageChanged(e.target.value)} placeholder="Écrivez votre message ici..." sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2.5 } }} />
          </div>
        )}
        <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '10px 14px', borderRadius: 10, background: '#f8fafc', border: '1px solid #e2e8f0', marginBottom: 4 }}>
          <input type="checkbox" checked={props.handled_custom_message} onChange={(e) => props.handledCustomMessageChanged(e.target.checked)} style={{ width: 16, height: 16, cursor: 'pointer' }} />
      ...
      <DialogActions style={{ padding: '12px 24px 20px', gap: 10 }}>
        <LoadingButton onClick={() => props.handledShowModalChanged(false)} variant="outlined" sx={{ textTransform: 'none', borderRadius: 2, borderColor: '#e2e8f0', color: '#64748b', fontWeight: 600 }}>Annuler</LoadingButton>
        <LoadingButton
